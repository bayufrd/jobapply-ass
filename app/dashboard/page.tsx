...
export default function DashboardPage() {
  return (
    <AppShell
      title="Dasbor"
      description="Pantau kampanye yang sedang berjalan, keputusan user yang tertunda, log otomatisasi terbaru, dan status auto apply dibantu saat ini."
      actions={
        <>
          <Link
            href="/cv/upload"
    ...
    >
      <div className="space-y-6">
        ...
        <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
                ...
                <div key={campaign.id} className="rounded-2xl border border-slate-800 bg-slate-950 p-4">
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-sm text-slate-400">Lowongan Saat Ini: {campaign.job}</p>
                    </div>
                  ...
                  <p className="mt-3 text-sm text-slate-300">Progress Target: {campaign.progress}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
              <h3 className="text-lg font-semibold">Aksi yang Perlu Ditangani</h3>
