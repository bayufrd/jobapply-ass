...
| Assisted Form Filling              | Partial | False manual_intervention pada `/apply/profile` sedang diperbaiki dengan guard URL-first. Flow Jobstreet internal memakai detector langkah URL sebagai state utama: `/apply` langsung lanjut tanpa isi field, `/apply/role-requirements` untuk pertanyaan employer, `/apply/profile` diperlakukan sebagai langkah normal update profil dan hanya mengisi field wajib visible yang benar-benar kosong, lalu lanjut ke `/apply/review`. Teks lemah seperti `Profile`, `Profile Avatar`, `Open app`, atau config tersembunyi tidak lagi boleh dianggap login/verifikasi. AI/MCP tetap hanya membaca elemen visible, membantu isi jawaban aman, dan mencari tombol Continue/Submit. | `lib/browser/jobstreet-apply-step-detector.ts`, `lib/browser/jobstreet-apply-step-runner.ts`, `lib/browser/page-detector.ts`, `lib/browser/ai-first-apply-runner.ts`, `lib/browser/mcp-ai-apply-runner.ts`, `lib/browser/jobstreet-apply-agent.ts` |
...
* URL-step detector Jobstreet: Detector URL adalah sumber kebenaran utama untuk `/apply`, `/apply/role-requirements`, `/apply/profile`, `/apply/review`, dan `/apply/success`. URL normal apply internal tidak boleh di-override menjadi `manual_intervention` kecuali ada bukti visible kuat.
* Manual intervention detector: [`detectManualIntervention()`](lib/browser/page-detector.ts:251) sedang diperketat agar hanya memakai visible text dan visible interactive elements. Password input, OTP input, captcha, halaman `/oauth/login`, dan security verification visible menjadi sinyal kuat; teks lemah seperti `Profile Avatar`, `Open app`, atau config tersembunyi tidak boleh memicu pause.
* AI-first apply runner: [`runAiFirstApplyRunner()`](lib/browser/ai-first-apply-runner.ts:1) kini mulai menulis log URL-step seperti `jobstreet_apply.step_detected_from_url`, `manual_intervention.candidate_detected`, `manual_intervention.false_positive_rejected`, dan `manual_intervention.confirmed` untuk membedakan kandidat false-positive vs intervensi manual nyata.
* MCP AI-first runner: [`runMcpAiApplyRunner()`](lib/browser/mcp-ai-apply-runner.ts:1) juga mulai membaca langkah Jobstreet dari URL snapshot MCP dan menulis log `jobstreet_apply.update_profile_started` sebelum planner AI berjalan.
* AI planner: AI dipakai spesifik untuk menemukan tombol Continue/Submit dan mengisi field kosong yang relevan, bukan untuk menebak seluruh flow saat URL sudah jelas.
...
Tanggal: 2026-06-02 (update dokumentasi setelah guard false manual_intervention `/apply/profile`, sebelum verifikasi final)
...
* [ ] Konfirmasi jika tidak ada field wajib kosong di `/apply/profile`, sistem menulis log `jobstreet_apply.update_profile_no_required_fields`, klik Continue, lalu URL berpindah ke `/apply/review`.
...
| 2026-06-02 | False manual_intervention pada langkah normal `/apply/profile` Jobstreet | Detector generic manual intervention masih bisa salah membaca kata `Profile`/elemen lemah sebagai login/verifikasi walau URL apply internal normal | In Progress | `lib/browser/jobstreet-apply-step-detector.ts`, `lib/browser/page-detector.ts`, `lib/browser/jobstreet-apply-step-runner.ts`, `lib/browser/ai-first-apply-runner.ts`, `lib/browser/mcp-ai-apply-runner.ts` |
