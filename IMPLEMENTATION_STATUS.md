...
| Campaign Loop / Autopilot v2       | Partial | Autopilot kini menyimpan dan memakai state paginasi kampanye (`currentSearchPage`, `currentSearchUrl`, `processedJobCount`, `unusableJobCount`, `emptyPageCount`, `lastAppliedJobstreetJobId`) agar dapat search → proses job satu per satu → pindah ke halaman berikutnya sampai target tercapai. Terminal step baru seperti `no_jobs_remaining_after_all_pages`, `too_many_empty_pages`, dan `target_reached` sudah diperlakukan sebagai controlled state. Untuk kasus URL lamaran invalid atau `external_redirect`, job sekarang ditandai terkontrol sebagai `apply_unavailable`/`external_redirect_invalid_url`, `appliedCount` tidak bertambah, `submitted` tidak dipalsukan, AI planner dilewati, dan loop lanjut ke lowongan eligible berikutnya tanpa crash global. Pada task ini, seluruh action client kampanye kini mengirim `actionName` ke [`readJsonSafely()`](components/campaign-actions.tsx:24) sehingga jika response kosong/invalid terjadi lagi, UI Bahasa Indonesia akan menampilkan endpoint dan aksi yang memicu error, bukan hanya `empty_response_body`. Route [`autopilot/start`](app/api/campaigns/[id]/autopilot/start/route.ts:16), [`autopilot/continue`](app/api/campaigns/[id]/autopilot/continue/route.ts:16), [`autopilot/decision`](app/api/campaigns/[id]/autopilot/decision/route.ts:4), [`qa/campaigns/[id]/run`](app/api/qa/campaigns/[id]/run/route.ts:14), dan [`local-log`](app/api/campaigns/[id]/local-log/route.ts:17) juga sudah dinormalisasi agar selalu mengembalikan JSON non-empty. Verifikasi browser nyata untuk target 1/5/100 masih belum selesai. | `lib/campaign/autopilot-runner.ts`, `lib/campaign/campaign-pagination-state.ts`, `app/api/campaigns/[id]/status/route.ts`, `components/campaign-actions.tsx`, `scripts/qa-autopilot.mjs`, `lib/api/json-response.ts`, `lib/utils/safe-json.ts` |
...
* [x] [`node --test ./tests/campaign-pagination-state.test.ts ./tests/campaign-state.test.ts ./tests/jobstreet-search-url.test.ts ./tests/jobstreet-url.test.ts ./tests/jobstreet-apply-step-detector.test.ts`](tests/jobstreet-search-url.test.ts:1) lulus.
* [x] [`node --experimental-strip-types --test ./tests/api-json-response.test.ts ./tests/safe-json.test.ts ./tests/safe-url.test.ts ./tests/jobstreet-url.test.ts`](tests/api-json-response.test.ts:1) lulus.
* [x] [`npx prisma generate`](prisma/schema.prisma:1) lulus.
* [x] [`npx tsc --noEmit`](package.json:45) lulus.
* [x] [`npm run lint`](package.json:10) lulus dengan warning lama yang tidak terkait perubahan task ini (`no-unused-vars` di [`lib/browser/ai-first-apply-runner.ts`](lib/browser/ai-first-apply-runner.ts:33), [`lib/browser/jobstreet-apply-agent.ts`](lib/browser/jobstreet-apply-agent.ts:32), [`lib/campaign/autopilot-runner.ts`](lib/campaign/autopilot-runner.ts:266), dan [`scripts/qa-autopilot.mjs`](scripts/qa-autopilot.mjs:330)).
* [x] Reproduksi [`curl -i http://localhost:3000/api/campaigns/cmpw0lf1q00009ktzw0h6qz32/status`](app/api/campaigns/[id]/status/route.ts:78) mengembalikan `404` JSON non-empty (`{"error":"Kampanye tidak ditemukan."}`).
* [x] Reproduksi [`curl -i -X POST http://localhost:3000/api/campaigns/cmpw0lf1q00009ktzw0h6qz32/autopilot/start`](app/api/campaigns/[id]/autopilot/start/route.ts:16) mengembalikan `500` JSON non-empty (`autopilot_start_failed`).
* [x] Reproduksi [`curl -i -X POST http://localhost:3000/api/campaigns/cmpw0lf1q00009ktzw0h6qz32/autopilot/continue`](app/api/campaigns/[id]/autopilot/continue/route.ts:16) mengembalikan `500` JSON non-empty (`autopilot_continue_failed`).
* [x] Reproduksi [`curl -i -X POST http://localhost:3000/api/qa/campaigns/cmpw0lf1q00009ktzw0h6qz32/run`](app/api/qa/campaigns/[id]/run/route.ts:14) mengembalikan `404` JSON non-empty (`campaign_not_found`).
* [x] Reproduksi [`curl -i http://localhost:3000/api/qa/campaigns/cmpw0lf1q00009ktzw0h6qz32/status`](app/api/qa/campaigns/[id]/status/route.ts:5) mengembalikan `404` JSON non-empty (`Kampanye tidak ditemukan.`).
* [ ] [`npm run qa:autopilot -- --campaign=cmpw0lf1q00009ktzw0h6qz32 --target=1`](package.json:15) belum bisa diverifikasi live karena campaign ID target tidak ada pada environment lokal saat ini.
...
1. Jalankan QA live dengan campaign ID yang benar-benar ada di environment lokal, lalu verifikasi apakah UI masih pernah menampilkan error diagnostik dari [`readJsonSafely()`](components/campaign-actions.tsx:24) dan catat endpoint pastinya bila masih ada.
2. Jalankan [`npm run qa:autopilot -- --campaign=cmpw0lf1q00009ktzw0h6qz32 --target=1`](package.json:15) dan kampanye error asli bila berbeda, lalu verifikasi tidak ada uncaught `Invalid URL`, job invalid/external dilewati aman, dan campaign lanjut ke lowongan berikutnya.
3. Verifikasi manual snapshot MCP live pada halaman Jobstreet nyata untuk memastikan parser accessibility text menghasilkan `buttons`, `questions`, dan `pageKind` yang lebih kaya, terutama saat `external_redirect` dan `unknown` page.
